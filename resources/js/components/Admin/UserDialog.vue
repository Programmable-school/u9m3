    ...
    <v-dialog v-model="dialog" max-width="650px" persistent>
        ...
        <v-card>
            <v-toolbar :color="titlecolor" dark>
                <v-toolber-title><v-icon class="pb-1">{{ $route.meta.icon }}</v-icon>{{ $route.mata.name }} | {{ title }}</v-toolber-title>
            </v-toolbar>

            <v-card-text>
                <v-container>
                    <v-layout column weap>
                    <v-text-field class="pb-3" label="名前" placeholder="氏名を入力してください"
                                    ...
                                    :disabled="type == 'D'"
                    ></v-text-field>
                    <v-text-field class="pb-3" label="ログインID" placeholder="社員IDを入力してください"
...
export default {
  ...
  data: () => ({
    ...
    rules: {
      requierd: value => !!value || "Required.",
      min2: value => value.length >= 2 || "Min 2 characters",
      min6: value => value.length >= 6 || "Min 6characters"
    }
  }),
  ...
  methods: {
    ...
    save() {
      ...
      //変更があった時だけ通信
      if (
        JSON.stringify(this.orig).replace(/[\s|]+/g, "") !==
        JSON.stringify(this.items).replace(/[\s|]+/g, "")
      ) {
        this.store();
      }
    ...
    open(item, flg) {
      ...
      if (this.type == "C") {
        ...
        (this.titlecolor = "primary"),
          (this.placeholder_password =
            "パスワードを指定してください(未指定の場合はログインIDを設定)");
      }
      ...
      if (this.type == "U") {
        ...
        (this.titlecolor = "accent"),
          (this.placeholder_password =
            "変更する場合はパスワードを指定してください(未指定の場合は変更しない)");
      }
    ...
    store() {
      ...
      params.append("pass", this.items.role ? 5 : 10);
      params.append("type", this.type);

      this.clearError();

      axios
        .post("/api/admin/user/store", params)
        ...
        .catch(
          function(error) {
            ...
            if (error.response && [401, 419].includes(error.response.status)) {
              ...
              error.response &&
              [422].includes(error.response.status)
            ) {
              alert(error.response.data.message);
    ...
    destroy() {
      ...
      axios
        .post("/api/admin/user/destroy", params)
