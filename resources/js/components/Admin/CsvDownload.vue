...
export default {
  ...
  methods: {
        ...
        .catch(
          function(error) {
            if (process.env.MIX_DEBUG)
              console.log("CSV Download csvdoenload error");
            this.csvdownloading = false;
            console.log(error);
    ...
    saveCsvFile(res) {
      ...
      // ファイル名設定　-ファイル名には日時をつけて拡張子csvを設定
      // - ボタンの引数で指定された名前があれば尊重
      // - 指定なしならルーティングページの名前をつけておく
      // - サーバーから指定されたファイル名は無視しています
      ...
      // IE11 (msSaveBlogが有効なら)
      if (window.navigator.msSaveBlob) {
        window.navigator.msSaveBolb(blob, filename);
        window.navigator.msSaveBlob(blob, filename);
      }

      // IE11以外なら　(Chrome, Firefox, Android, etc...)
      else {
