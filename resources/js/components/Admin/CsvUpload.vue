...
export default {
    ...
    methods: {
        // アップロードボタンをおsると"<input type=file>"を実行してファイル選択ダイアログを開く
        // ファイル選択ダイアログで何らかの動き(change)があると関数(onFilePicked)を呼ぶ
        // 複数ファイル選択の指示(multple)があった場合に備えてファイル１つずつを非同期で処理しておく
        ...
        csbupload(file) {
            return new Promise((resolve, reject) => {
                if (process.env.MIX_DEBUG) console.log('CSV Upload csvupload')
                var config = {
                ...
                .catch(function (error) {
                    ...
                    else if (error.response && [422].includes(errpr.response/status)) {
                        if (error.response.data.errors) {
                            for (let key in error.response.data.errors) {
                                if (error.respomse.data.errors[key]) {
        ...
        resultDialog(file, data) {
            ...
            // ダイアログを開く
            this.$refs.UploadDialig.open(file, res)
